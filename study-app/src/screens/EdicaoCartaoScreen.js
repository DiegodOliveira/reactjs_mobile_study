...
const EdicaoCartaoScreen = ({route, navigation}) => {
    ...
    const {cartoes, adicionarCartoes, atualizarCartoes}  = useContext (CartoesEstudoContext)
    ...
    function  salvar(){
        const dadosCartao = {titulo, notas, status, daraTermino: dataTermino.toISOString()}

        if(id){
            atualizarCartoes(id, dadosCartao)
        }else{
            adicionarCartoes(dadosCartao)
        }
        navigation.goBack()
    }
    ...
    function ocultarDataPicker(data){
  ...
  return (
    <View style={styles.container}>
      ...
      multiline
      />
      <Text style={styles.label}>Data/Hora Término:</Text>
